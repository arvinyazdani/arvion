SECTIONS = [
    ...
    ("integrations", "هلو و اتصال‌ها", "تا اطلاعات فنی نباشد، تعهد اتصال قطعی نباید در قرارداد نوشته شود.", [
        ("holoo", "نسخه و نوع نصب هلو چیست؟", "تک‌کاربره یا شبکه‌ای، محل نصب، نوع دیتابیس و امکان API یا Export را بنویسید."),
        ("sync", "چه داده‌هایی بین سیستم‌ها جابه‌جا شود؟", "طرف حساب، کالا، پیش‌فاکتور، فاکتور، مانده و پرداخت را مشخص کنید."),
        ("source", "مرجع نهایی داده و مسئول رفع مغایرت چه کسی است؟", "فاصله همگام‌سازی و مسئول تأیید مغایرت را مشخص کنید."),
    ]),
