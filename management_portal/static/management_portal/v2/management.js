(() => {
  ...
  /* Inbox quick actions stay inside the installed app. Network failures use
     the original POST as a progressive-enhancement fallback. */
  const asyncToast = document.querySelector("[data-async-toast]");
  let asyncToastTimer = 0;
  const showAsyncToast = (message, tone = "success") => {
    if (!asyncToast) return;
    window.clearTimeout(asyncToastTimer);
    asyncToast.textContent = message;
    asyncToast.dataset.tone = tone;
    asyncToast.hidden = false;
    asyncToastTimer = window.setTimeout(() => {
      asyncToast.hidden = true;
    }, 4500);
  };
  const updateUnreadBadges = (count) => {
    ...
    document.querySelectorAll("[data-notification-link]").forEach((link) => {
      const prefix = isPersianDocument ? link.dataset.labelFa : link.dataset.labelEn;
      link.setAttribute("aria-label", isPersianDocument
        ? `${prefix}؛ ${safeCount} خوانده‌نشده`
        : `${prefix}; ${safeCount} unread`);
    });
  };
  const updateQueueAfterRemoval = (card) => {
    const queue = card.closest("[data-notification-queue]");
    const items = queue?.querySelector("[data-notification-items]");
    if (!queue || !items) return;
    card.remove();
    const remaining = items.querySelectorAll("[data-notification-card]").length;
    const count = queue.querySelector("[data-queue-count]");
    if (count) count.textContent = String(remaining);
    if (!remaining && !items.querySelector(".notice-empty-mini")) {
      const empty = document.createElement("p");
      empty.className = "notice-empty-mini";
      empty.textContent = queue.dataset.emptyText || (isPersianDocument ? "موردی برای نمایش نیست." : "Nothing to show.");
      items.appendChild(empty);
    }
  };
  const finishNotificationForm = (form) => {
    form.removeAttribute("aria-busy");
  ...
  document.querySelectorAll("form[data-notification-action]").forEach((form) => {
    form.addEventListener("submit", async (event) => {
      ...
      if (!response.ok || !payload.ok) {
        finishNotificationForm(form);
        showAsyncToast(payload.message || (isPersianDocument ? "انجام این اقدام ممکن نشد." : "This action could not be completed."), "error");
        return;
      }

      const card = form.closest("[data-notification-card]");
      updateUnreadBadges(payload.unread_count);
      showAsyncToast(payload.message);
      if (!card) {
        finishNotificationForm(form);
        return;
      }
      if (payload.action === "claim") {
        ...
        if (owner) owner.textContent = payload.owner || "—";
        card.querySelector('form[data-notification-action="claim"]')?.remove();
      }
      if (payload.status === "read") {
        card.classList.remove("unread");
        card.classList.add("read");
        card.querySelector('form[data-notification-action="read"]')?.remove();
      }
      if (payload.status === "resolved") {
        card.classList.add("is-resolving");
        const delay = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ? 0 : 180;
        window.setTimeout(() => updateQueueAfterRemoval(card), delay);
