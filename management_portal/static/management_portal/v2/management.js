(()=>{
  ...
  const menu=document.querySelector('.m-menu'),bottomMenu=document.querySelector('.m-bottom-menu'),sidebar=document.querySelector('.m-sidebar'),scrim=document.querySelector('.m-scrim');
  if(menu&&sidebar&&scrim){const close=()=>{sidebar.classList.remove('open');scrim.classList.remove('open');menu.setAttribute('aria-expanded','false')};const toggle=()=>{const open=!sidebar.classList.contains('open');sidebar.classList.toggle('open',open);scrim.classList.toggle('open',open);menu.setAttribute('aria-expanded',String(open))};menu.addEventListener('click',toggle);if(bottomMenu)bottomMenu.addEventListener('click',toggle);scrim.addEventListener('click',close);document.addEventListener('keydown',event=>{if(event.key==='Escape')close()})}
  ...
  const enable=async(localTest=false)=>{
    ...
    if(localTest)await registration.showNotification(fa?'تست اعلان آرویون':'Rvion notification test',{body:fa?'اگر این پیام را می‌بینید، نمایش اعلان روی دستگاه سالم است.':'If you can see this message, on-device notifications work.',tag:'rvion-local-test',icon:'/static/core/icons/icon-192.png',badge:'/static/core/icons/icon-192.png',data:{url:location.pathname}});
    if(help)help.textContent=localTest?(fa?'تست محلی ارسال شد؛ اکنون باید اعلان تست روی همین دستگاه دیده شود.':'Local test sent; a test notification should now appear on this device.'):(fa?'این دستگاه با موفقیت برای همه رویدادهای مدیریتی ثبت شد.':'This device is registered for all management events.');label(fa?'اعلان فعال است — ارسال تست':'Alerts enabled — send test','ready');
  };
  buttons.forEach(button=>button.addEventListener('click',()=>enable(true).catch(()=>{if(help)help.textContent=fa?'اتصال یا نمایش تست کامل نشد. تنظیمات اعلان دستگاه را بررسی کنید.':'Setup or local display failed. Check device notification settings.';label(fa?'تلاش دوباره':'Try again','error')})));
  if('Notification'in window){if(Notification.permission==='granted')enable(false).catch(()=>label(fa?'تلاش دوباره':'Try again','error'));else if(Notification.permission==='denied')explainBlocked();}
})();
