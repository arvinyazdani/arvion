(()=>{
  ...
  const mobileRequest=document.querySelector('.m-bottom a[href*="/management/requests/"]');if(mobileRequest){const lang=document.documentElement.lang;mobileRequest.href=`/${lang}/management/crm/`;const icon=mobileRequest.querySelector('b'),text=mobileRequest.querySelector('span');if(icon)icon.textContent='◉';if(text)text.textContent=lang==='fa'?'CRM':'CRM'}
