(() => {
  ...
  const previousLabel = form.querySelector("[data-previous-label]");
  const stepNames = ["کلینیک", "هدف و فرآیند", "نوبت و پرداخت", "آموزش و وبینار", "فناوری و اجرا"];
  ...
  const show = (index, focus = true) => {
    ...
    if (nextLabel && current < steps.length - 1) nextLabel.textContent = `بعدی: ${stepNames[current + 1]}`;
    if (previousLabel && current > 0) previousLabel.textContent = `بازگشت به ${stepNames[current - 1]}`;
